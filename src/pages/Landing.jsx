      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer' }} onClick={() => navigate('/')}>
          <div style={{
            ...
            boxShadow: '0 4px 12px rgba(91,76,245,0.25), inset 0 1px 1px rgba(255,255,255,0.2)'
          }}>
            <span style={{ color: 'white', fontWeight: 800, fontSize: 18, fontFamily: 'var(--font-mono)' }}>A</span>
          </div>
          <span style={{ fontWeight: 700, fontSize: 19, color: 'var(--text-heading)', letterSpacing: '-0.02em' }}>AURA</span>
        </div>

        {/* Links & CTA */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <button onClick={() => navigate('/track')} className="btn-ghost" style={{ border: 'none', padding: '10px 16px' }}>
            Track Status
          </button>

          {/* Citizen auth — Register or My Dashboard */}
          {isLoggedIn ? (
            <button onClick={() => navigate('/citizen')} className="btn-ghost" style={{ padding: '10px 20px', background: 'white', color: 'var(--accent)', fontWeight: 600 }}>
              My Dashboard
            </button>
          ) : (
            <>
              <button onClick={() => navigate('/login')} className="btn-ghost" style={{ border: 'none', padding: '10px 16px' }}>
                Citizen Login
              </button>
              <button onClick={() => navigate('/register')} style={{
                padding: '9px 20px', borderRadius: 'var(--r-full, 9999px)',
                background: 'var(--accent-light, #EEF0FF)', color: 'var(--accent)',
                border: '1px solid rgba(91,76,245,0.2)', fontSize: 14, fontWeight: 600, cursor: 'pointer',
                transition: 'all 0.2s ease',
              }}
                onMouseEnter={e => { e.currentTarget.style.background = 'var(--accent)'; e.currentTarget.style.color = 'white' }}
                onMouseLeave={e => { e.currentTarget.style.background = 'var(--accent-light, #EEF0FF)'; e.currentTarget.style.color = 'var(--accent)' }}
              >
                Register Free
              </button>
            </>
          ...
          <button onClick={() => navigate('/officer/login')} className="btn-ghost" style={{ padding: '10px 20px', background: 'white' }}>
            Officer Login
          </button>
          <button onClick={() => isLoggedIn ? navigate('/submit') : navigate('/register')} className="btn-primary" style={{ padding: '10px 24px' }}>
            File a Complaint
          </button>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '60px 32px', width: '100%', position: 'relative', zIndex: 1 }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 0.8fr', gap: 64, alignItems: 'center', position: 'relative', zIndex: 1 }}>
          ...
          <div>
            ...
            {/* Language Ticker */}
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1, duration: 1 }} style={{ display: 'flex', alignItems: 'center', gap: 16, background: 'rgba(255,255,255,0.6)', padding: '12px 20px', borderRadius: 100, border: '1px solid rgba(0,0,0,0.05)', maxWidth: 540 }}>
              <Globe size={16} style={{ color: 'var(--text-tertiary)' }} />
              <div className="ticker-wrap" style={{ flex: 1 }}>
                <div className="ticker-inner" style={{ display: 'inline-flex', gap: 24, paddingRight: 24 }}>
                  {[...BHASHINI_LANGUAGES, ...BHASHINI_LANGUAGES].map((l, i) => (
                    <span key={i} style={{ fontSize: 13, color: 'var(--text-secondary)', fontWeight: 500 }}>{l.nativeName}</span>
                  ))}
                </div>
              </div>
            </motion.div>
