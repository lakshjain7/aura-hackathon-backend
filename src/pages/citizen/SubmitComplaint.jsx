...
function InlineLanguagePicker({ currentLang, onSelect }) {
  const scrollRef = useRef(null)
  const [showSheet, setShowSheet] = useState(false)
  const isMobile = typeof window !== 'undefined' && window.innerWidth < 640

  if (isMobile) {
    const current = BHASHINI_LANGUAGES.find(l => l.code === currentLang) || BHASHINI_LANGUAGES[0]
    return (
      <>
        <motion.button
            ...
            <>
              ...
              >
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                  <span style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-primary)' }}>Select Language</span>
                  <button onClick={() => setShowSheet(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-tertiary)' }}>
                    <X size={18} />
                  </button>
                </div>
                  ...
                  {BHASHINI_LANGUAGES.map(lang => {
                    ...
                    return (
                      <motion.button
                        key={lang.code}
                        whileTap={{ scale: 0.95 }}
                        onClick={() => { onSelect(lang.code); setShowSheet(false) }}
                        style={{
                      ...
                      >
                        <span style={{ fontSize: 14, fontWeight: 500, color: selected ? 'white' : 'var(--text-primary)' }}>{lang.nativeName}</span>
                        <span style={{ fontSize: 10, color: selected ? 'rgba(255,255,255,0.7)' : 'var(--text-tertiary)', marginTop: 2 }}>{lang.script}</span>
                      </motion.button>
                    )
                  })}
                </div>
        ...
        {BHASHINI_LANGUAGES.map(lang => {
          ...
          return (
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              animate={selected ? { boxShadow: '0 2px 8px rgba(91,76,245,0.25)' } : { boxShadow: '0 0px 0px rgba(0,0,0,0)' }}
              whileHover={!selected ? { y: -1, boxShadow: 'var(--shadow-sm)', borderColor: 'var(--accent)' } : {}}
              style={{
            ...
            >
              <span style={{ fontSize: 14, fontWeight: 500, color: selected ? 'white' : 'var(--text-primary)', whiteSpace: 'nowrap' }}>{lang.nativeName}</span>
              <span style={{ fontSize: 10, color: selected ? 'rgba(255,255,255,0.7)' : 'var(--text-tertiary)', marginTop: 2 }}>{lang.script}</span>
            </motion.button>
          )
        })}
      ...
      try {
        ...
        recorder.onstop = async () => {
          ...
          try {
            ...
            updateField('text', res.data.transcription || res.data.text || '')
          } catch {
      ...
      <div style={{ background: 'var(--bg-muted)', minHeight: '100vh', paddingBottom: 120 }}>
        <div style={{ maxWidth: 680, margin: '0 auto', padding: '80px 24px 0' }}>
          ...
          <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} style={{ marginBottom: 32 }}>
            ...
            <InlineLanguagePicker currentLang={currentLang} onSelect={setLanguage} />
          </motion.div>
